...
import sys
sys.path.append(str(Path(os.getenv("OPENSTUDIO_EXE")).parents[1] / "Python"))
sys.path.append(str(Path(os.path.abspath(__file__)).parents[1]))
import openstudio

from measure import ModifyStochasticFilePython


class TestModifyStochasticFilePython:
    """Py.test module for ModifyStochasticFilePython."""

    def test_number_of_arguments_and_argument_names(self):
        """Test that the arguments are what we expect."""
        # create an instance of the measure
        measure = ModifyStochasticFilePython()
        ...
        assert arguments[0].name() == "csv_path"
    ...
    def test_good_argument_values(self):
        ...
        # you don't need it in the dict
        args_dict = {}
        args_dict["csv_path"] = "stochastic.csv"  # path to the CSV file to be modified
        ...
        assert result.info()[0].logMessage() == "CSV file stochastic.csv was modified."
